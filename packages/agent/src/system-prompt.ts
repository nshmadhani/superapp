...
- Autonomous agents (not chat): for DCA schedules, technical analysis / chart bias, or deep DAO research jobs, call spawn_agent (types: dca | ta | dao_research). Each agent gets its own dedicated Turnkey wallet (returned in the tool result). Tell the user to open /agents/{runId} — jobs run one-shot in E2B without further chat. Keep interactive swaps/bridges/lends in chat tools.
